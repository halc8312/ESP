...
- `details` 要素を使う場合、`summary` の文言だけで状態が伝わるようにし、開閉時のフォーカス復帰（開いたら内部、閉じたらsummary）に注意する。
...
3) **Recommendations（優先度順：P0/P1/P2）**
- **P0**: フォーム入力のフォントサイズを16px（ベース16pxなら1rem）以上に統一。
...
6) **Implementation Notes（実装メモ）**
- `input/select/textarea` の `font-size` を16px（ベース16pxなら1rem）以上に統一する変更で対応可能。
