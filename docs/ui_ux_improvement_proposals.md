...
- `details` 要素を使う場合、`summary` に状態変化が伝わるラベル/`aria-expanded`/`aria-controls` の付与と、開閉時のフォーカス復帰（開いたら内部、閉じたらsummary）に注意。
...
3) **Recommendations（優先度順：P0/P1/P2）**
- **P0**: フォーム入力のフォントサイズを16px相当（1rem以上）に統一。
...
6) **Implementation Notes（実装メモ）**
- `input/select/textarea` の `font-size` を1rem以上に統一する変更で対応可能。
