...
| 4b    | DB スクレイピング層 Selenium 削除 | ✅ 完了    | `docs/specs/STAGE_4_RESULTS.md` 参照      |

### 1-2. Stage 4b 完了メモ

Stage 4b は 2026-03-10 に完了済みです。現在は以下の状態です。

- `yahoo_db.py`, `offmall_db.py`, `snkrdunk_db.py`, `yahuoku_db.py`, `surugaya_db.py` から Selenium 依存を削除済み
- `services/patrol/*_patrol.py` の Selenium デッドコードを削除済み
- `requirements.txt` から `selenium`, `webdriver-manager`, `undetected-chromedriver` を削除済み
- `Dockerfile` から Chrome 導入処理を削除済み
- 完了記録は `docs/specs/STAGE_4_RESULTS.md` に集約
...
`reports.md` 記載の改善提案（原件数ベース）：

| 分類 | 機能数 | 備考 |
|------|--------|------|
| 商品一覧ページ (1-1 〜 1-11) | 11 | 一部完了。正確な進捗はセクション 8 のチェックリストを参照 |
| 商品編集ページ (2-1 〜 2-3)  | 3  | 一部完了。正確な進捗はセクション 8 のチェックリストを参照 |
| 商品抽出ページ (3-1 〜 3-6)  | 6  | 一部完了。正確な進捗はセクション 8 のチェックリストを参照 |
| 価格表管理ページ (4-1 〜 4-3) | 3  | 未着手 |
| **合計**                     | **23** | **進捗管理はセクション 8 を正とする** |
...
- [x] **A-1** `debug_scrape.py`, `debug_children.py`, `debug_variant_json.py` 削除
- [x] **A-2** 各 patrol ファイルの `_fetch_with_selenium()` デッドコード削除
  - [x] `services/patrol/yahoo_patrol.py`
  - [x] `services/patrol/offmall_patrol.py`
  - [x] `services/patrol/snkrdunk_patrol.py`
  - [x] `services/patrol/yahuoku_patrol.py`
  - [x] `services/patrol/surugaya_patrol.py`
- [x] **A-3** `yahoo_db.py` Selenium 完全除去（`create_driver` 削除含む）
- [x] **A-4** `offmall_db.py`, `snkrdunk_db.py`, `yahuoku_db.py` Selenium 除去
- [x] **A-5** `surugaya_db.py` Selenium フォールバック除去
- [x] **A-6** `requirements.txt` から `selenium`, `webdriver-manager`, `undetected-chromedriver` 削除
- [x] **A-6** `Dockerfile` から Chrome インストールブロック削除
- [x] `pytest tests/ -v` 全通過確認
- [x] `docs/specs/STAGE_4_RESULTS.md` 作成

### Block B: UI 改善 Phase 1

- [x] **B-1** eBay 関連項目削除
- [x] **B-2** テーブル列整理（サイト削除、画像枚数削除、ステータス→在庫、元URL→抽出サイト）
- [x] **B-3** 価格列二段表示（仕入/販売）
- [x] **B-4** 検索フィルタ コンパクト化（PC・モバイル統合）
- [x] **B-5** 「スクレイピング」→「商品抽出」文言変更
- [x] **B-6** ローディング画面追加
- [x] **B-7** 検索フォーム コンパクト化
- [x] **B-8** 商品編集ページ コンパクト化

### Block C: 機能追加 Phase 2

- [x] **C-1** 英語タイトル・説明フィールド追加（DB マイグレーション）
- [x] **C-2** インライン編集 API（selling_price, custom_title_en）
- [x] **C-3** 一括価格設定 API
- [x] **C-4** 抽出結果 同画面表示 + 選択登録
- [x] **C-5** 画像削除・並べ替え（SortableJS）
