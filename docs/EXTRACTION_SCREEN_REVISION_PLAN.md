...
作成日: 2026-06-10 / 更新: 2026-06-10（クライアント回答を反映） / 対象リポジトリ: halc8312/ESP
...
### フェーズ4: リストの時間指定自動非公開 + 仕上げ
20. `PriceList.unpublish_at`（任意の日時）を additive migration で追加。公開カタログ(`routes/catalog.py`)で期限超過リストを非公開扱いにする（自動削除はしない）
21. リスト編集UIに非公開日時の設定欄と状態表示（公開中/非公開済み）を追加
22. ドキュメント更新（README / AGENTS.md の feature reality 更新）
23. Render本番での動作確認（esp-worker 経由のジョブ実行）

### 将来アップデート候補（今回スコープ外、クライアント要望あり）
- 翻訳除外ワード登録：店舗独自ワード（例「ポイント10倍」）やサイト固有ワード（例「プロフ確認」）を翻訳前に除去/置換するユーザー辞書機能
- 翻訳の手動一括修正UI：自動適用後に目視確認→一括/個別修正（個別修正は既存の商品編集で可能。一括修正は追加実装）
- サイト別・リスト別の価格ルール切替

---

## 4. クライアント回答（2026-06-10）と確定方針

| 確認事項 | 回答 | 確定方針 |
|---|---|---|
| 1. キーワード検索タブ | 当面残しでOK。ただし中古品は各サイト側で絞り込んだ方が効率的 | タブは残しつつ、URL貼り付け（検索結果URL）をメイン導線として設計 |
| 2. リストのみ登録商品のパトロール対象化 | **未回答** | 暫定：監視対象外として実装（負荷軽減）。後から有効化できる設計にする |
| 3. 翻訳の自動適用 | 自動適用でOK。適用後に目視確認→手動で一括/個別修正したい。将来的に除外ワード登録で最適化したい | `auto_apply` で実装。個別修正は既存の商品編集で対応。一括修正UI・除外ワードは将来アップデート候補として記録 |
| 4. 利益ルールの粒度 | デフォルトルール1つ + 手動一括/個別設定でOK。必要になったら追加 | デフォルト `PricingRule` 1つをユーザー設定に追加。個別上書きは既存の manual_margin_rate / manual_shipping_cost を活用 |
| 5. 一時リストの有効期限 | 自動削除ではなく**時間指定の自動非公開**がよい | `PriceList.unpublish_at` を追加し、期限超過で公開カタログから非公開（データは保持） |

その他：クライアント提供のDeepBay（検索URL抽出型の類似サービス）のログイン情報が利用可能。参考調査・テスト抽出可（テスト時はテスト用の名前を付けること）。
