...
### Next Step: バリエーション管理UIの強化（優先度: 中）

現在、データベースとCSV出力は複数バリエーションに対応しましたが、UI上では「既存バリエーションの編集」しかできません（新規行の追加機能がない）。
アパレルなどで「S, M, L」を一括で追加したり、行を増やしたりするUIの実装が必要です。

1.  **バリエーション行の追加・削除**: UI上で動的に行を増減させるJavaScript機能の実装。
2.  **バリエーション一括生成**: 「サイズ: S, M, L」と入力すると自動で3行生成されるようなウィザード機能。

### その他 検討事項
- **認証機能**: 現在はログイン機能がないため、Basic認証やFlask-Loginの導入を検討。
- **画像編集**: トリミングやリサイズ機能。
