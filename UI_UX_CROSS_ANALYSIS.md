...
> **修正状況（2026-08-13 更新）**
>
> **全26件のうち24件が対応済み**です。「推奨する着手順」の15項目に加え、
> P3-18 / 19 / 21 / 22 / 23 / 25 と P2-16 の残りも対応しました。
> 回帰テストは `tests/test_ui_regressions.py`（47件）にあります。
>
> **未対応で残しているのは2件**です。
> - **P3-24**（公開カタログの細かい引っかかり）: 通貨選択の保存、価格フィルタの通貨追従、
>   レート表示の書式ゆれ、`lang="en"`。仕様判断が要るものを含むため未着手
> - **P3-26**（デザイントークンの定着）: テンプレート側のハードコード色 約230箇所。
>   `trash.html` の独自の赤だけは `.btn-danger` に寄せました
>
